...
url1 = 'https://www.shanbay.com/api/v1/vocabtest/vocabularies/?' + datas[choice][0]
res = requests.get(url1, headers=headers)
#print(res.status_code)
voc_json = res.json()
vocabularies = voc_json['data']

list_vocabularies = []
for v in vocabularies:
    list_vocabularies.append(v['content'])

words_knows = []
not_knows = []

for i in range(50):
    print( str(i) + '  ' + list_vocabularies[i] + '\n')
    x = input('认识输入1，不认识输入0：')
    if x == '1':
        words_knows.append(list_vocabularies[i])
    else:
        not_knows.append(list_vocabularies[i])

print('你认识' + str(len(words_knows)) + '个单词\n分别是：')

for i in range(len(words_knows)):
    print(words_knows[i])
